...
const roll = (msg: string, advantage: boolean): string => {
    const msgComponents: string[] = msg.split(" ").splice(1);
    const diceResult: number[] = diceRoll(msgComponents[0], advantage);
    const diceSum: number = advantage
        ? diceResult.reduce((previous, current) => current > previous ? current : previous)
        : diceResult.reduce((previous, current) => previous + current, 0);
    const restOfArray: string[] = msgComponents.slice(1);
    const finalResult: number = calculateRest(diceSum, restOfArray);
    if (advantage) {
        return `Etulyöntiasemaheiton tulos on **${finalResult}**! Sinä heitit **${diceResult.join(", ")}**!`;
    }
...
const diceRoll = (dice: string, advantage: boolean): number[] => {
    let diceComponents: string[] = dice.split("d");
    let multiplier: number;
    multiplier = advantage ? 2 : ((diceComponents[0] != "") ? +diceComponents[0] : 1)
